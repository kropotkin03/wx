...
class MyApp(App):
    def OnInit(self):
        ...
        for i in range(len(liAsis)):
            ...
            #cCosto.Bind(EVT_BUTTON, self.cargaCosto(i))
    ...
    def cargaBD(self, cA):
        ...
        if dlgCargaBD.ShowModal() == ID_OK:
            self.cargaSQL(listaA)



    def cargaSQL(self, lista):
        con = sqlite3.connect("asistentes.db")
        cur = con.cursor()
        try:
            cur.execute("CREATE TABLE asist (Id INTEGER PRIMARY KEY, Nombre TEXT)")
        except:
            pass
        for x in range(len(lista)):
            cur.execute("INSERT INTO asist (Nombre) VALUES(?)", [lista[x].GetValue()])
        con.commit()
        con.close()
    ...
    def recupBD(self):
        con = sqlite3.connect("asistentes.db")
        cur = con.cursor()
        cur.execute("SELECT Nombre FROM asist ORDER BY Nombre")
        rows = cur.fetchall()
        listaA = []
        for row in rows:
            listaA.append(row[0])
