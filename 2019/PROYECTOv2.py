...
class MyApp(App):
    ...
    def liquidos (self):
        ...
        caja.Add(boton, 1)
        self.search = search = SearchCtrl(self.panel1) #Boton buscar
        search.ShowCancelButton(True)
        relleno = StaticText(self.panel1) #El relleno espara qe el botòn Buscar se ubique en el lado opuesto del botòn Agregar
        boton.Bind(EVT_BUTTON, self.agregarLiquidos)
        caja.Add(relleno,4, flag=EXPAND)
        caja.Add(search, flag=EXPAND)
