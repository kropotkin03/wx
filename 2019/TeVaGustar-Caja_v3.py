...
class MyApp(App):
    def OnInit(self):
        self.op = "a"
        f = Frame(None, title="Te va gustar", size=(1100,600))
        p = self.p = Panel(f, -1)
...
#Guarda el participante
    def guardaPart(self, evt):
        ...
        self.dvlc.AppendItem(["0", no, self.fe, re, en, fp, sa, te]) 
        print(self.fe)       
        self.grabaBD(no, self.fe, re, en, fp, sa, te)
        self.f2.Close()
...
# Begin Calendario
    def OnDateChanged(self, evt):
        ...
        print("->", self.fe, "<-")
# End Calendario
    def grabaBD(self, pnombre, pfecha, pretira, pentrega, pformadepago, psaldo, ptelefono):#Graba en la base de datos
